...
#                 "homemade", "sammon", "isomds", "cmdscale", "shepard". 
#             "homemade" projection just use y axis for rows and x axis for
#             columns. For example in the case cited above, it will put
#             x=(1+3)/2=2 and y=(2+3)/2=2.5. Default is "homemade".
...
#
# NOTE:Still a very buggy version
#             
bubbleplot=function(A, row1,col1, row2=c(), col2=c(), row3=c(), col3=c(), fichero="resultado.txt", projection="homemade", showLabels=FALSE)
  {
  #0) Length checking
  numBic2=0
  numBic3=0
  ...
  if(length(col3)>0)
    ...
    if(length(col2)>0)
      ...
      if(length(col3)>0)
        ...
        #20,21 - círculo lleno/hueco
        #16,23 - rombo lleno/hueco
        #22 - cuadrado
...
#  ssCol=paleta[((ss-min(ss))/(max(ss)-min(ss)))*length(paleta)]
#  ssFich=(ss-min(ss))/(max(ss)-min(ss))
...
#  distanciasCon=dist(unique(con))
 ...
 #tipos=c(rep(16,numBic1),rep(23,numBic2), rep(22,numBic3))
  ...
  if(projection=="shepard")
    {
    smdGen=Shepard(distanciasGen, k=1)
    smdCon=Shepard(distanciasCon, k=1)
    puntos=cbind(smdCon$points, smdGen$points)  
    todo=cbind(smdCon$points, smdGen$points, tamaños, ssFich, tipos)
    }
  if(projection=="homemade")
...
#  plot(puntos, cex=tamaños/10, col=ssCol, pch=tipos)
  if(showLabels==TRUE)
    {
    if(projection=="homemade" || projection=="cmdscale")
      {
      #text(smdCon, smdGen,pos=3, labels=etiquetas, cex=(1.5+(1/ss)))
      text(smdCon, smdGen,pos=3, labels=etiquetas, cex=(1.5))
      }
    else
      {
      text(smdCon$points, smdGen$points,pos=3, labels=etiquetas, cex=(1.5))
      #text(smdCon$points, smdGen$points,pos=3, labels=etiquetas, cex=(1.5+(1/ss)))
      }
    }
  }
